...
/**
 * @author Nihal
 * This class formats the data crawled from plataformaotan.blogspot.com.es to a format that blogtrackers supports
 */

public class PlataformaOtan {
	...
	public PlataformaOtan() {
		// TODO Auto-generated constructor stub
		this.query = "SELECT * FROM ukraine_db.temp_plataformaotan where POSTPERMALINK not like '%search?updated%' and POSTPERMALINK != 'http://www.plataformaotan.blogspot.com.es/' and FLAG != 2";
		r = new Resources();
	}
	...
	public String run() throws Exception
	{
		...
		for(int i=0;i<count;i++)
		{
			...
			if(insert)
			{
				...
				LinkedList<String> outLinks = po.linkExtractor(temp);
				int outLinksNum = outLinks.size();
				//System.out.println(postpermalink[i] + "\t" + outLinksNum);

				String date1 = po.dateConverter(date[i]);
				...
				String insertQuery = "insert into blogtrackers.blogposts (title,date,blogger,categories,post,"
						...
						postpermalink[i]+"\",55,\""+tag+"\")";

				r.runQuery(insertQuery);

				query = "UPDATE ukraine_db.temp_plataformaotan set FLAG = 2 where POSTPERMALINK =\'" + postpermalink[i] 
						+ "\'";
				r.runQuery(query);
		...
		query = "UPDATE blogtrackers.blogsites SET last_crawled = \"" + lastCrawl + "\" where blogsite_id = 55";
		r.runQuery(query);
		conn.close();
	 ...
	 * This method identifies proper outlinks by removing links that are tags or advertisements or social media links
	 * 
	 */
