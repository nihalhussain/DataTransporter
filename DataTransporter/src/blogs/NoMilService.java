...
/**
 * @author Nihal
 * This class formats the data crawled from nomilservice.com to a format that blogtrackers supports
 */

public class NoMilService {
	...
	public NoMilService() {
		...
		this.query = "select * from ukraine_db.temp_nomilservice where POSTPERMALINK != 'http://www.nomilservice.com/' and POSTPERMALINK not like '%search?updated%' and FLAG != 2";
		r = new Resources();
	...
	public String run() throws Exception
	{
		...
		for(int i=0;i<count;i++)
		{
			...
			if(insert)
			{
				...
				LinkedList<String> outLinks = nms.linkExtractor(temp);
				int outLinksNum = outLinks.size();

				String date1 = nms.dateConverter(date[i]);
				...
				String insertQuery = "insert into blogtrackers.blogposts (title,date,blogger,categories,post,"
						...
						postpermalink[i]+"\",48,\""+tag+"\")";

				r.runQuery(insertQuery);
				
				query = "UPDATE ukraine_db.temp_nomilservice set FLAG = 2 where POSTPERMALINK =\'" + postpermalink[i] 
						+ "\'";
				r.runQuery(query);
		...
		query = "UPDATE blogtrackers.blogsites SET last_crawled = \"" + lastCrawl + "\" where blogsite_id = 48";
		r.runQuery(query);
		conn.close();
