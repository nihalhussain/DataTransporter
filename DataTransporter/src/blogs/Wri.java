...
/**
 * @author Nihal
 * This class formats the data crawled from wri-irg.org to a format that blogtrackers supports
 */
...
public class Wri {
	...
	public Wri() {
		...
		this.query = "SELECT * FROM ukraine_db.temp_wri_irg where POSTPERMALINK not like '%/campaigns/%' and POSTPERMALINK not like '%/index.%' and FLAG != 2";
		r = new Resources();
	}
	...
	public String run() throws Exception
	{
		...
		for(int i=0;i<count;i++)
		{
			...
			if(insert)
			{
				...
				LinkedList<String> outLinks = wri.linkExtractor(temp);
				int outLinksNum = outLinks.size();
				//System.out.println(postpermalink[i] + "\t" + outLinksNum);
				
				String date1 = wri.dateConverter(date[i]);
				...
				String insertQuery = "insert into blogtrackers.blogposts (title,date,blogger,categories,post,"
						...
						postpermalink[i]+"\",54,\""+tag+"\")";

				r.runQuery(insertQuery);
				
				query = "UPDATE ukraine_db.temp_wri_irg set FLAG = 2 where POSTPERMALINK =\'" + postpermalink[i] 
						+ "\'";
				r.runQuery(query);
		...
		query = "UPDATE blogtrackers.blogsites SET last_crawled = \"" + lastCrawl + "\" where blogsite_id = 54";
		r.runQuery(query);
		conn.close();
