...
/**
 * @author Nihal
 * This class formats the data crawled from tgnapau.blogspot.com to a format that blogtrackers supports
 */

public class Tgnapau {
	...
	public Tgnapau() {
		...
		this.query = "SELECT * FROM ukraine_db.temp_tgnapau where POSTPERMALINK != 'http://tgnapau.blogspot.com.es/' and POSTPERMALINK not like '%search?update%' and FLAG != 2";
		r = new Resources();
	}
	...
	public String run() throws Exception
	{
		...
		for(int i=0;i<count;i++)
		{
			...
			if(insert)
			{
				...
				LinkedList<String> outLinks = tg.linkExtractor(temp);
				int outLinksNum = outLinks.size();
				//System.out.println(postpermalink[i] + "\t" + outLinksNum);
				
				String date1 = tg.dateConverter(date[i]);
				...
				String insertQuery = "insert into blogtrackers.blogposts (title,date,blogger,categories,post,"
						...
						postpermalink[i]+"\",50,\""+tag+"\")";

				r.runQuery(insertQuery);
				
				query = "UPDATE ukraine_db.temp_tgnapau set FLAG = 2 where POSTPERMALINK =\'" + postpermalink[i] 
						+ "\'";
				r.runQuery(query);
		...
		query = "UPDATE blogtrackers.blogsites SET last_crawled = \"" + lastCrawl + "\" where blogsite_id = 50";
		r.runQuery(query);
		conn.close();
	...
	public String dateConverter(String date)
	{
		...
		months.put("de gener de", "01");
		months.put("de febrer de", "02");
		months.put("de març de", "03");
		months.put("d’abril de", "04");
		months.put("de maig de", "05");
		months.put("de juny de", "06");
		months.put("de juliol de", "07");
		months.put("d’agost de", "08");
		months.put("de setembre de", "09");
		months.put("d’octubre de", "10");
		months.put("de novembre de", "11");
		months.put("de desembre de", "12");
